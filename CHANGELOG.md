...
- **feat(i18n): translate the reverted CMD migration wizard**
  - Merging `develop` into the revert of the CMD account-linking flow kept the
    reverted wizard's own texts hardcoded in Portuguese, while the rest of the
    login area had moved to `t("migration.*")` keys. The wizard now uses i18n
    keys throughout: existing keys are reused where the string survives the
    revert (`password`, `checking`, `verify`, `back`, …) and nine new keys
    (`migrateTitle`, `verifyPasswordTitle`, `migrateSuccessTitle`, …) cover the
    revert-specific wording — "migrar" instead of "associar", "palavra-passe
    antiga" — in both `pt` and `en`, fixing the missing accents along the way.
