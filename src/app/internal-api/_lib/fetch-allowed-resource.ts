...
/**
 * Shared SSRF-hardened upstream fetch for the internal-api proxy routes
 * (CSV preview and spreadsheet preview).
 *
 * Security model (TICKET-60 / VULN-2079) — kept identical across every
 * proxy that relays a remote resource on behalf of the browser:
 *
 *   1. Allowlist of hostnames AND ports — the original prefix check
 *      `url.startsWith("https://dados.gov.pt")` was bypassable with
 *      `https://dados.gov.pt.s.inty.io` (audit PoC). We compare the parsed
 *      `URL.hostname` against an exact allowlist and require an allowed port.
 *
 ...
 *      between "validate hostname" and "open TCP connection".
 *
 *   3. Streamed body cap — the response is read chunk-by-chunk and aborted
 *      once it exceeds `maxBytes`, instead of loading the full upstream body
 *      into memory before slicing.
 *
 *   4. Content-Type allowlist — only the caller-provided prefixes are
 *      accepted; the proxy is not a generic relay for arbitrary content.
 *
 *   5. Redirect refused (`redirect: "error"`) — bypass via 3xx to a different
 *      host is blocked at the WHATWG Fetch level.
 *
 *   6. Identifiable User-Agent on outbound requests.
 *
 *   7. Production guardrail — an allowlist entry resolving to a private IP is
 *      logged loudly; the request itself is refused by the IP-validation step.
 */

const DEFAULT_ALLOWED_HOSTS = ["dados.gov.pt", "preprod.dados.gov.pt"];
const FETCH_TIMEOUT_MS = 10_000;
const USER_AGENT = "dadosgov-proxy/1.0 (+https://dados.gov.pt)";
...
const PRIVATE_V6: RegExp[] = [/^::1$/, /^fc/, /^fd/, /^fe[89ab]/i];
...
export function logProxyEvent(label: string, event: Record<string, unknown>) {
  ...
  // filter by `event=<label>`. Avoids leaking the resolved IP into user-facing
  // responses but emits it in the operator-facing log.
  console.log(JSON.stringify({ event: label, ...event }));
}

export interface FetchAllowedOptions {
  /** Content-Type prefixes accepted from the upstream response. */
  allowedContentTypePrefixes: string[];
  /** Hard cap on the streamed body, in bytes. */
  maxBytes: number;
  /** Value sent in the outbound `Accept` header. */
  accept: string;
  /**
   * Accept a response that carries no `Content-Type` header at all. Some
   * allowlisted storage backends (e.g. the dados.gov.pt `/s/resources/` store)
   * serve files with `nosniff` and no content-type; the body is still
   * size-capped and the caller validates it (e.g. by parsing). Defaults to
   * false — an explicit unsupported content-type is always rejected.
   */
  allowMissingContentType?: boolean;
  /** Requester IP for structured logs (best-effort). */
  requesterIp: string;
  /** Log event label (e.g. "proxy-csv", "proxy-spreadsheet"). */
  logLabel: string;
}

export type FetchAllowedResult =
  | {
      ok: true;
      bytes: Uint8Array;
      total: number;
...
/**
 * Validate `rawUrl` against the SSRF allowlist, fetch it with the resolved IP
 * pinned, enforce the content-type and size caps, and return the raw bytes.
 * Callers decide how to serialize the body (text decode vs. binary parsing).
 */
export async function fetchAllowedResource(
  rawUrl: string | null,
  opts: FetchAllowedOptions
): Promise<FetchAllowedResult> {
  const t0 = Date.now();
  const { requesterIp, logLabel, maxBytes } = opts;
  ...
  // 3. DNS resolution + IP validation. The resolved IP is then pinned for the
  // actual fetch via a custom `lookup`, protecting against DNS rebinding.
  ...
  try {
    ...
    return { ok: false, status: 502, error: "Failed to resolve upstream host" };
  }

  let res: Response;
  try {
    // Pin the resolved IP for the upcoming TCP connection. Node's `fetch`
    // accepts `dispatcher` (undici) for this; overriding the URL's hostname
    // would break TLS SNI / certificate validation on HTTPS, so we keep the
    // original URL and use undici's connect options.
    const { Agent } = await import("undici");
    // undici >= 7 expects the lookup callback in the array form
    // `cb(err, [{address, family}])`, not the legacy node:net
    // `cb(err, address, family)` shape.
  ...
  // 4. Content-Type allowlist. A missing/empty content-type is accepted only
  // when the caller opts in (the body is still capped and validated downstream).
  const contentType = (res.headers.get("content-type") || "").toLowerCase();
  const ctAllowed =
    (contentType === "" && opts.allowMissingContentType === true) ||
    opts.allowedContentTypePrefixes.some((p) => contentType.startsWith(p));
  if (!ctAllowed) {
    logProxyEvent(logLabel, {
      level: "warn",
      url: target.toString(),
    ...
    return { ok: false, status: 415, error: "Upstream returned unsupported content-type" };
  }

  // 5. Streamed read with hard size cap. Avoids reading a huge body into
