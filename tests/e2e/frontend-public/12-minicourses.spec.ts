...
const MINICOURSES_URL = "/recursos/aprender/minicursos";
...
test.describe("Mini-Courses Page", () => {
  ...
  test("MC-01: Page loads with banner, search and course cards", async ({
    ...
    const cards = page.locator("a[href^='/recursos/aprender/minicursos/']");
  ...
  test("MC-04: Cards expose href slugs", async ({ page }) => {
    ...
    const firstCard = page.locator("a[href^='/recursos/aprender/minicursos/']").first();
    await expect(firstCard).toBeVisible({ timeout: 15000 });

    const href = await firstCard.getAttribute("href");
    expect(href).toMatch(/^\/recursos\/aprender\/minicursos\/[a-zA-Z0-9-]+$/);
  });

  test("MC-05: Click card opens course detail", async ({ page }) => {
    const firstLink = page.locator("a[href^='/recursos/aprender/minicursos/']").first();
    await expect(firstLink).toBeVisible({ timeout: 15000 });
    await firstLink.click();
    await page.waitForURL(/\/recursos\/aprender\/minicursos\/.+/, { timeout: 15000 });
